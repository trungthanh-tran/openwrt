...
class FakeRunner:
    ...
    def __init__(self, results=None, has_config=False):
        self.calls = []
        self.results = results or {}
        self.uploads = []          # (local path, remote path)
        self.remote_sizes = {}     # remote path -> bytes delivered
        self.has_config = has_config  # a wifi-socks.conf was already on the router
...
class ProvisionRunnerTests(unittest.TestCase):
    ...
    def installed_router(self, **extra):
        ...
        return FakeRunner(results, has_config=True)
