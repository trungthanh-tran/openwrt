#!/bin/sh
# set-sock.sh — change one Wi-Fi's proxy endpoint without reloading Wi-Fi.
# Updates wifi-socks.conf (the source of truth), then regenerates sing-box config.
#
# Usage:
#   scripts/set-sock.sh <idx> <proxy_host> <proxy_port> [user] [pass] [socks5|http]
# Example:
...
IDX="$1"; HOST="$2"; PORT="$3"; USER="${4:-}"; PASS="${5:-}"; PROXY_TYPE="${6:-socks5}"
[ -n "$IDX" ] && [ -n "$HOST" ] && [ -n "$PORT" ] || die "Usage: set-sock.sh <idx> <host> <port> [user] [pass] [socks5|http]"
...
case "$PROXY_TYPE" in socks5|http) :;; *) die "proxy type must be socks5 or http" ;; esac
...
awk -F'|' -v OFS='|' -v idx="$IDX" -v h="$HOST" -v p="$PORT" -v u="$USER" -v pw="$PASS" -v pt="$PROXY_TYPE" '
  /^#/ || NF<10 { print; next }
  { gi=$3; gsub(/ /,"",gi) }
  gi==idx { $5=h; $6=p; $7=u; $8=pw; $12=pt; print; next }
  { print }
' "$CONF" > "$TMP"
mv "$TMP" "$CONF"
validate_conf
...
log "Changed $PROXY_TYPE proxy for Wi-Fi idx=$IDX -> $HOST:$PORT"
