...
# The console routes on the URL fragment (index.html#devices), so one page is
# served. Earlier installs dropped a full copy of the console at each workspace
# name; those copies carry their own inline JS and would keep serving whatever
# release they were installed at, so they are removed rather than left behind.
for page in config devices analytics settings status egress diagnose maintenance; do
  rm -f "/www/sbproxy/${page}.html"
done
