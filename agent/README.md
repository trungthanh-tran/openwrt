...
| GET | `debug` | — | Trợ lý gỡ lỗi: quét toàn router theo luật, trả `{summary, verdict, findings[]}` xếp nặng trước; mỗi finding có `fix` là tên cách sửa (rỗng nếu phải tự xử lý) và song ngữ `title/title_en` |
| POST | `debug_fix` | `{id}` | Chạy ĐÚNG một cách sửa đã được đặt tên: `singbox_restart`, `config_eol`, `bridge_nf`, `apply`, `install_agent`. Id chỉ nhận `[a-z_]`; tên khác bị từ chối trước khi gọi script |
