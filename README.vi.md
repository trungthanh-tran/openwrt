...
- **Theo dõi traffic theo host** (`TRAFFIC_STATS=1` + `scripts/traffic.sh`): host nào đang tải bao nhiêu qua proxy, kèm `--suggest` sinh luật bypass.
- **Gán proxy cho máy trên dải LAN chính** (`LAN_PROXY=1`): máy cắm dây và SSID dùng chung dải LAN cũng ghim proxy được, qua pool `idx 0`.
- **Luật định tuyến theo domain/IP** (`config/routing-rules.conf`): cho một đích đi thẳng, chặn hẳn, hoặc ép qua proxy. Không có file = mọi thứ qua proxy như trước.
...
- **Gói mang đi** — Windows x64 có file `.exe` standalone chạy ngay và gói `.zip`
  đầy đủ; Linux dùng `.tar.gz`. Gói đầy đủ có deployer, router update package,
  checksum, tài liệu và ảnh minh hoạ. Xem
  [hướng dẫn Web Deployer](docs/WEB-DEPLOYER.md) và
  [cấu trúc release](docs/RELEASE-ARTIFACTS.md).
- **Desktop Console đầy đủ** — tải
  `sbproxy-console-<version>-windows-x64.exe` để quản lý trực tiếp SSID, proxy,
  thiết bị, gateway, backup và log. File `sbproxy-web-deployer-*.exe` chỉ dùng
  để kiểm tra/cài/cập nhật Web Console.
...
- **[docs/RELEASE-ARTIFACTS.md](docs/RELEASE-ARTIFACTS.md) — cấu trúc và cách build Windows standalone/ZIP và Linux TAR.GZ.**
- **[docs/WEB-DEPLOYER.md](docs/WEB-DEPLOYER.md) — chạy Web Deployer, cài/cập nhật router và xử lý lỗi bằng hình ảnh.**
